...
可以使用 `make new [component]` 创建新组件 <sup>

[1](#user-content-fn-make)

</sup>

。创建组件会调用脚本自动创建模板文件并维护组件编译时相关配置。
...
打包组件会把 package 目录下所有组件 package/<span>
...
.js。由于 components.json 聚合了所有组件的路径信息，所以会在各打包任务中用到。
...
测试 webpack.component.js，可以使用以下指令：
...
为了减小引入样式的体积，组件样式也可以单独引入。在第二步 gulp build 时，会将 SCSS 样式统一编译为 CSS，并用第三步的 cp-cli<sup>

[2](#user-content-fn-cp-cli)
...
按照官网的介绍，**想要按需载入组件以减小项目打包体积，可以使用 babel-plugin-component**<sup>

[3](#user-content-fn-babel-plugin-component)
...
component
 ...
 及 lib/theme-chalk/<span>

component

</span>

.css 也就拿到组件独立的资源文件了。相比整个引入 Element，单独引入组件自然减小了资源体积。
...
1. 可以把 Make 理解为一种类似 Gulp 的项目工程化工具，用于编写一些供命令行调用执行的任务。运行 Make 指令需要安装特定软件。 [↩](#user-content-fnref-make)
2. 拷贝操作依赖了 cp-cli@npm，可以简单理解为拷贝指令的 NodeJS 版。 [↩](#user-content-fnref-cp-cli)
3. 见 Element 官网，quickstart 部分：[https://element.eleme.cn/#/zh-CN/component/quickstart#an-xu-yin-ru](https://element.eleme.cn/#/zh-CN/component/quickstart#an-xu-yin-ru) [↩](#user-content-fnref-babel-plugin-component)
