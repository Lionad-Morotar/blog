# 🌪️ Tailwind v4 如何兼容预处理器

> tailwind4 不支持 sass 的解决方法
...
UnoCSS 是一个原子化 CSS 引擎，在某种程度而言，他和 tailwind 是竞品关系。但是比 tailwind 更先进的一点体现在他的设计得非常灵活和可扩展，可以通过定义规则来支持各种用法，且包含转化器，
可以解析并处理 @apply 等指令。
...
然后，在 main.css 或其他样式入口，降低 UnoCSS 的输出层优先级。默认 UnoCSS 输出到 default 层，所以在入口文件使用 layer 语法预先定义 default 层和 tailwind 层（base、
component、utils）的优先级，如下代码。
