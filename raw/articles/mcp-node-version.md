# 🔧 Claude Code MCP 集体报错排查记录

> 记录一次因 Node.js 版本导致 Claude Code 多个 MCP server 报 Connection closed 的排查过程
...
理所当然，我第一反应是：直接指定某个 Node.js 版本的 npx 路径就行，比如：
...
所以得换方向，写一个 shell 脚本，把所有要执行的内容放到脚本里，然后在脚本顶部定义好我们需要的环境变量，这样能锁定脚本执行的 Node.js 版本。

见以下代码，wrapper 的内容可以写死 PATH，exec 则用来透传剩下的所有参数。
