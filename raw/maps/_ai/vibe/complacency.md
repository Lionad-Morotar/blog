...
> AI 编程助手提高效率的同时，会放大重复代码、代码 churn 和审查难度，需要用工程实践和流程设计来对冲”惰性依赖”风险
...
综合来看，原文的核心立场并不是”反对 AI 编程”，而是明确提醒：**AI 提速的是”编码”这一局部环节，但代价是在系统层面增加了质量管理的复杂度和认知风险**。要在生产中有效使用这类工具，技术负责人需要把 TDD、静态分析、
review 规范等实践重新”嵌入工作流”，并通过共享指令和流程设计，把团队的质量文化编码进 AI 使用方式，而不是寄希望于”更聪明的模型自动抵消这些风险”。

#### AI 驱动工程文化中的运营退化与”衔尾蛇”循环

- L1: 2026年3月31日 Claude Code 源码泄露暴露的运行时问题包括：7 个进程占用 5.3 GB RAM，单个进程峰值分配 36.5 GB，另一进程 5 分钟内堆内存达 93 GB；同月发生 8 次服务中断。
`autoCompact.ts` 中记录约 1,279 个会话出现 50+ 连续失败（最高达 3,272 次），每天浪费约 25 万次 API 调用，修复仅 3 行代码。
- L1: 问题追踪系统高度自动化：估计 26,792 个 issue 关闭中有 49-71% 由机器人驱动；Issue #38335 获 201 个赞、被标为”invalid”、零团队回应。
- L2: Anthropic 首席工程师 Boris Cherny 对泄露的回应是”通过更快、更多自动化和让 Claude 检查结果来解决问题”——这被解读为一种结构性信号：当 AI 系统失败时，组织的本能反应是引入更多 AI，
而非更多人类审查。
- L3: 作者主张这是一种”衔尾蛇”（ouroboros）状态——AI 写代码、AI 审查代码、AI 检查部署；当系统崩溃时，答案是更多 AI。100% AI 贡献率从来不是产品指标，而是营销叙事。

见：Denis Stetskov, “The Snake That Ate Itself”（2026-04-01）

#### AI 编码的终极价值判断——更好而非更快

- L2: AI 放大的是组织已有的东西——良好的工程纪律会产出更优质的代码，而缺失的纪律会以机器速度转化为技术债务。
- L3: 作者主张，”销售 AI 编程工具的公司无法用自家的 AI 编程工具打造出高质量产品”。未来的工程需要的是更好的工程（better engineering），而非更快的工程（faster engineering）。
