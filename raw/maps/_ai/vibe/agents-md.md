...
> AGENTS.md 是一种用于向 AI 编码代理提供说明的常用格式

## Brief

AGENTS.md 是一种常用格式，用来向参与项目的 AI 编码代理提供说明。<br />
...
#### Context File 真的有效吗？一项针对 6 万个仓库的研究

一项覆盖 60,000 个开源仓库的实证研究揭示了 Context File（如 AGENTS.md、CLAUDE.md、.cursorrules）的真实效果：
**效果取决于谁写的**。
...
1. **LLM 生成的 Context File 有害**：额外的上下文迫使推理模型"思考得更辛苦"，却没有产生更好的最终补丁，反而降低成功率并显著增加成本
2. **人工编写有效但有边界**：仅当包含**不可推断的细节**时才有效，例如高度特定的工具使用方式或自定义构建命令。包含架构概述等信息并未减少模型定位相关文件的时间
3. **成本普遍上升**：无论谁写的 Context File，都会增加 token 消耗。LLM 生成的成本增幅更大

**实践建议：**

- 完全省略 LLM 生成的 Context File
- 人工编写的指令应仅限于不可推断的细节
