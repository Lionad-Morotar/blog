...
> Vibe 是一种基于 AI 的代码生成和协作方式，旨在通过自然语言提示帮助开发者快速生成代码片段、自动化任务和提升编程效率。
...
Karpathy 把个人感受里的“程序员投降时刻”锚定到 2025 年 12 月。在那之前他把 Cursor 等工具当作“会出错的助手”，需要不断修正；之后最新模型生成的代码块开始直接可用，他在 side project 中放弃逐轮纠正，
进入完全凭感觉编程的状态。他自述记不清上一次需要修改 AI 输出是什么时候。
...
- [Loop Engineering](/maps/_ai/vibe/loop-engineering) — 从手动提示到自主循环系统的设计范式

#### AI 辅助开源贡献的分工模式

Matteo Collina 使用 Claude Code 在假期完成 Node.js core 的 `node:vfs` PR（14,000 行代码，66 个文件），展示了 AI 辅助大型开源项目的典型分工：人类负责架构设计、
API 设计和逐行 code review；AI 处理所有繁琐实现（fs 方法的 sync/callback/promise 三种变体）、测试覆盖和文档生成。
...
Symphony 的 Section 10.5 是典型案例：关于 GraphQL 扩展的"规格"只是缺乏连贯性的约束罗列。这是典型的 agent 工作产品特征：缺乏目的感和对更大图景的理解。更深层的问题是劳动价值的贬低——
当企业从"规格工作应该比编码更容易"的前提出发，就无法完成规格写作所需的困难思考。
...
David Abram 认为软件开发的真正价值从不在于写代码本身，而在于理解系统、调试无解问题、设计可承载高负载的架构，以及做出能避免数月痛苦的决策。这些核心能力无法被 LLM 替代——它们无法理解系统全貌，无法在心中保持上下文，
更无法判断决策的对错。

> 最核心的是：它们不会做出选择。那部分仍然属于你。软件开发真正的工作，真正让一个人有价值的部分，是知道什么东西应该存在，以及为什么。
> —— David Abram, The machine didn't take your craft. You gave it up.

我对此存疑。

见：[A quote from David Abram](https://simonwillison.net/2026/Mar/23/david-abram/)
