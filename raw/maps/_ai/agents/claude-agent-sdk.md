...
> Anthropic Claude Agent SDK 的使用方式、调用栈与 User-Agent 机制
...
由于 User-Agent 只是普通 HTTP header，在 Claude Code 的自定义 header 机制中会被最终请求采用，覆盖掉 CLI 默认生成的 User-Agent。
Anthropic 侧对客户端的识别并不依赖 User-Agent，而是依赖 system prompt 中的 billing header，因此修改 User-Agent 不会影响订阅计费或客户端身份判定。
