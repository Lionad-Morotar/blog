# OpenAI Agents SDK (JavaScript) 发布记录

> OpenAI Agents SDK JavaScript/TypeScript 版本的完整发布历史与变更日志
...
此版本新增对 OpenAI Responses API 的 WebSocket 模式支持。这是一个新的可选功能，默认行为保持不变。如需将所有 Responses API 调用切换为 WebSocket 模式，
请调用 `setOpenAIResponsesTransport('websocket')` 为所有 OpenAI Responses 模型代理启用。
