...
- [⚠️ 重要限制：
Skills 不跨平台同步](#%EF%B8%8F-%E9%87%8D%E8%A6%81%E9%99%90%E5%88%B6-skills-%E4%B8%8D%E8%B7%A8%E5%B9%B3%E5%8F%B0%E5%90%8C%E6%AD%A5)
...
代码编写原则与 Skill 编写原则存在系统性差异。Perplexity Agents 团队指出，Python 之禅中至少一半的格言在 Skill 语境下完全错误或具有误导性。<sup>
...
<tbody>
  ...
  <tr>
    ...
    <td>
      Activation is implicit pattern matching. Progressive disclosure.
    </td>
  </tr>
  ...
  <tr>
    ...
    <td>
      Context is expensive. Maximum signal per token.
    </td>
  </tr>
...
Insight：Skill 不是为程序员编写的软件，而是为模型构建的上下文。写得好代码的工程师往往需要重新校准直觉——许多对代码有用的模式在 Skill 中会变成反模式。
...
不是每个领域都需要 Skill。Perplexity 团队的标准是：先让 agent 在没有 Skill 的情况下跑几个 hero query，看它是否 already doing a good job。<sup>
...
- 模型已知的通用知识。例如，写一系列 git 命令的执行顺序——模型已经知道怎么做，写成 Skill 只能当文档，当不了 Skill
- 系统提示词已覆盖的内容。面向大多数请求的知识应放在全局上下文，而非条件加载的 Skill
- 变化速度快于维护速度的内容。例如远程 MCP endpoint 的工具列表频繁变化时，不应将其注入 Skill，否则会产生 drift

**每个 Skill 都是税**：每个 session、每个用户都要为 Skill 的 token 付费。对 Skill 中每句话的测试是："没有这条指令，agent 会做错吗？"如果不会，这条指令不能待在那里。
...
对于涉及大量知识领域的复杂 Skill，单一扁平目录可能导致模型选择困难。Perplexity 在 Computer 的美国所得税 Skill 中采用了三层嵌套：将 IRS 的 1,945 个 section 按逻辑细分为 20 个主题域，
每个域下再分具体 topic。<sup>
...
关键发现：早期测试表明，将所有 1,945 个 section 放在单个文件夹中的性能甚至比不加载 Skill 更差。组织成逻辑层级对高精度读取操作不可或缺。

但层级并非免费：每增加一层，信息架构的策展成本就上升一级。需要配套快速参考指南、自定义搜索工具等辅助机制，帮助模型以最小间接性定位信息。
...
- 推荐使用动名词形式命名（动词 + ing），清晰描述 Skill 提供的活动或能力，如 processing-pdfs、analyzing-spreadsheets、managing-databases、testing-code、
...
description 不是 Skill 的内部文档，而是模型的路由指令——这是 Skill 中最难写的一行。好的 description 不说"这个 Skill 做什么"，而说"何时加载这个 Skill"。<sup>
...
- 以 "Load when..." 开头，目标 50 词以内
- 描述用户意图，而非 Skill 功能
- 使用工程师在沮丧时会说的词，如 "babysit"、"watch CI"、"make sure this lands"
- 包含 2-3 个来自真实查询的示例，覆盖 80-20 场景

每增加一个 Skill，就有让其他所有 Skill 稍微变差的风险（action at a distance），因此 description 的精确性至关重要。
...
向 LLM 传达工作流程与向同事传达完全不同。对于存在一年以上的软件工具，只需提及其名称，LLM 就拥有所需信息。不要像在 README 中那样列出每个命令。<sup>
...
模型对后者的处理能力远强于前者过度规定性的命令序列，尤其当事情出错时。不要 railroad（过度规定），而应保持灵活。好的文档对模型来说通常是坏文档。

**专注 gotchas 和负例**：这些是极高信号的内容，因为它们指导模型"不要做什么"。每次 agent 绊倒时添加一行，gotchas 会自然生长。条件或内容极重的部分应从 SKILL.md（hub）移至 spoke 文件。
...
Perplexity 团队将写 evals 作为 Step 0，在写 Skill 内容之前完成。<sup>
...
- 真实用户查询：从生产环境或 brain trust 中采样
- 已知失败：agent 因缺少该 Skill 而失败的具体案例
- 邻域混淆：接近你的领域边界但路由到另一个 Skill 的案例

**负例极其重要**。负例（forbidden loads）有时比正例更有价值：确保 Skill 不会在不相关时被加载，避免 action at a distance 导致其他 Skill 性能退化。

**跨模型测试**：不同 orchestration 模型对 Skills 的行为差异显著。Sonnet 和 GPT 在 Skill loading 上的表现差异很大，eval suites 应在所有支持的模型上运行。
...
- 在 [SKILL.md](http://SKILL.md) 中引用参考文档时，使用描述性的链接文本，例如 “[详细的 API 端点说明见 API 参考](api-reference)"
...
<tbody>
  ...
  <tr>
    ...
    <td>
      <a href="http://SKILL.md" rel="nofollow">
        SKILL.md
      </a>
  ...
  <tr>
    ...
    <td>
      通过 bash 执行的文件，不占用上下文
    </td>
  </tr>
</tbody>
</table>

Perplexity 的三层成本模型与这里的分层完全对应，但更强调成本意识：<sup>
...
- **Index 层门槛极高**：~100 tokens/Skill 看似不多，但这是每个 session、每个用户始终支付的固定成本。Computer 的系统提示词中只注入 name 和 description，
模型据此决定是否调用 `load_skill()`
- **Load 层持续计费**：Skill 加载后，~5000 tokens 的正文会在 compaction boundary 之前始终留在上下文中。一个对话若加载 3-5 个 Skill，成本会相乘。
fluff 会 degrade 其他 Skill 的能力和整体 agentic 表现
- **Runtime 层最宽松**：scripts/、references/、assets/、subskills 只在 agent 读取时才加载，bar 最低，可以包含 20,000 tokens 或零 tokens
...
- 例如，在 [SKILL.md](http://SKILL.md) 中写 “对于复杂的表单填写场景，参见 [表单指南](FORMS)“，而不是在 [SKILL.md](http://SKILL.md) 中直接写 10 页的表单说明
...
- **高效的脚本执行**：当 Claude 运行 `validate_form.py` 时，脚本代码永远不会加载到上下文窗口。只有脚本的输出（如 “验证通过” 或特定错误消息）消耗 token。
这使得脚本比让 Claude 即时生成等效代码要高效得多
...
### Model Airplane：拟像应用作为领域知识载体

PostHog Wizard 团队跨多框架/语言维护一组「模型飞机」（Model Airplane）——**不是真生产应用，而是真应用的 simulacrum（拟像）**。关键特征是：外形完整（登录页、Stripe 接入、
用户事件等关键业务表面都存在），但行为简化（Auth-shaped 但任意密码都能登录，无真实业务逻辑）。

为什么这种「不那么精致」的拟像比真生产应用更适合作为 Skill 的领域知识载体：

- **Token 经济**：远比真应用紧凑，可以全量装进 Skill 的 Level 3 资源层
- **形状识别**：Agent 看到「Auth shape」就知道在哪里插入 PostHog 登录事件追踪
- **一致性强制**：让模型每次都补完相同形状的集成——15,000 月活用户得到的是同一种 PostHog 集成，而不是 15,000 种

**注入机制**：上下文服务（context service）把所有 Model Airplanes 扁平化为单个 Markdown 文件，作为 reference 与文档（plain text prose）一起装入 Skill。
这是对渐进式披露 Level 3 资源层的一种特殊填充——把「成功集成的形状」也作为可挂载的领域知识。

与「捕获隐性知识」「捕获决策逻辑」是同一系列模式：**Model Airplane 捕获的是「正确形状」**，而非具体步骤；适用于集成类、SDK 接入类 Skill 的领域知识捕获。
...
Skill 维护遵循 append-mostly 模式，gotchas 部分随时间累积最大价值。<sup>
...
- Agent 在某件事上失败 → 添加一个 gotcha
- Agent off-target 加载 Skill → 收紧 description，添加负例 evals
- Agent 该加载时未加载 → 添加关键词和正例 evals
- 系统提示词变更 → 检查 contention 或 duplication

gotchas 是负例，不直接改变显式指导，但让模型知道"这里有个已知失败"。从 80-20 走向 99.9% 成功率的过程中，gotcha 列表会自然增长——这是健康的，意味着 Skill 在从真实失败中学习。

**维护纪律**：如果修改 description，必须配套 evals 支持变更。description 决定 Skill 的路由，改动 description 而不改 evals 就是 off track。
...
1. [Designing, Refining, and Maintaining Agent Skills at Perplexity](https://research.perplexity.ai/articles/designing-refining-and-maintaining-agent-skills-at-perplexity): Perplexity 研究团队公开的生产级 Skill 工程指南，涵盖 Skill 目录结构、三层成本模型、description 路由触发器设计、eval 驱动开发和 gotcha 飞轮维护机制。 [↩](#user-content-fnref-1) [↩<sup>
...
](#user-content-fnref-1-4) [↩<sup>

5

</sup>

](#user-content-fnref-1-5) [↩<sup>

6

</sup>

](#user-content-fnref-1-6) [↩<sup>

7

</sup>

](#user-content-fnref-1-7) [↩<sup>

8

</sup>

](#user-content-fnref-1-8)
