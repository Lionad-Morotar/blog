...
vLLM、TensorRT-LLM 等系统处理 `temp=0` 时路径并不统一：部分框架走特殊贪心分支，另一部分仅用一个极小的浮点数替代温度除法<sup>

[1](#user-content-fn-1)

</sup>

。更根本的是，GPU 并行计算中的浮点累加顺序本身就不确定，
batch size 或显卡型号的改变都可能导致 softmax 后出现不同的 winner<sup>
...
PagedAttention 的内存优化优势会被它吃掉一部分<sup>
...
业内常见的工程权衡是用足够大的 Top-K（如 1024）来近似 Top-P 的效果，或者使用 CUDA 近似 Top-K kernel 绕过全排序<sup>

[4](#user-content-fn-4)

</sup>

。这是一个文档不会写、但上线后 profiler 会告诉你的决策点。
...
1. vLLM 与 TensorRT-LLM 对 `temperature=0` 的实现路径差异，详见各框架源码中 sampling 模块 [↩](#user-content-fnref-1)
2. GPU 浮点累加顺序的非确定性，参见 NVIDIA CUDA 编程指南中关于 floating-point associativity 的说明 [↩](#user-content-fnref-2)
3. PagedAttention 内存优化与采样计算开销的权衡，见 [vLLM PagedAttention 论文](https://dl.acm.org/doi/10.1145/3600006.3613165) [↩](#user-content-fnref-3)
4. CUDA 近似 Top-K kernel 方案，见 NVIDIA cuANN 及 vLLM 社区讨论 [↩](#user-content-fnref-4)
