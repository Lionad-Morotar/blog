...
> Adam Argyle 分享的 25+ 个 CSS 新特性，涵盖视图过渡、容器查询、滚动驱动动画、颜色系统等
...
通过 `view-transition-name` 可让侧边栏/导航栏在页面切换时保持不动，看起来像 SPA。
配合 JavaScript API `document.startViewTransition()` 可实现 DOM 变化时的动画过渡。
