...
配置里写一行 `CustomLog "|rotatelogs /var/log/access.%Y%m%d.log 86400" common`。Apache 启动时，管道符 `|` 告诉它：别写文件，启动 rotatelogs 进程，
把日志喂给它。rotatelogs 拿到数据后，按你指定的规则（每天、每小时、每 100MB）切分。
...
两种模式：时间间隔（秒数，如 `86400` 表示每天）或文件大小（字节数）。时间到了或文件大了，关闭当前文件，打开新文件。文件名支持 strftime 格式，
所以 `/var/log/access.%Y%m%d.log` 在 2024 年 3 月 15 日会变成 `/var/log/access.20240315.log`。
