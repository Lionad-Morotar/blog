...
> 前端开发各阶段的工具集合，包括 UI 框架、元框架、组件方案、动画库等。
...
> We have several options for choosing a store. Astro recommends the nanostores library in their documentation,
> which is what we'll use for our example, but several UI frameworks already come with a built-in solution—
> Svelte has Stores, Preact has Signals (as does Solid), and Vue has the Reactivity API.
