...
> Monorepo 是一种将多个项目放在同一个代码仓库中的管理方式，旨在提高团队协作和代码复用效率
...
使用 lerna bootstrap --use-workspaces 指令将会使 lerna 代理包管理器的依赖 linking 过程（类似 yarn 的 pnp 模式）；如果不带 --use-workspaces 指令，
将会使用传统的符号链接进行依赖管理。
...
![Command Execution](https://mgear-image.oss-cn-shanghai.aliyuncs.com/image/other/20221005225436.png)
...
Turborepo 是 Vercel 推出的 Monorepo 构建系统，专注于提供极速的构建体验。它通过智能的任务管道、本地缓存和远程缓存机制，大幅提升 Monorepo 项目的构建效率。

详细内容见：[Turborepo](/maps/_workflow/monorepo/turborepo)
