...
const { getSourceMapsEnabled, maybeCacheSourceMap, rekeySourceMap } = require('internal/source_map/source_map_cache')
...
const manifest = getOptionValue('--experimental-policy') ? require('internal/process/policy').manifest : null
...
const { encodedSepRegEx, packageInternalResolve } = require('internal/modules/esm/resolve')
...
function applyExports(basePath, expansion) {
    ...
    if (typeof pkgExports === 'object') {
        ...
        for (const candidateKey of ObjectKeys(pkgExports)) {
            ...
            if (candidateKey.length > dirMatch.length && StringPrototypeStartsWith(mappingKey, candidateKey)) {
        ...
        if (dirMatch !== '') {
            ...
            const resolved = resolveExportsTarget(pathToFileURL(basePath + '/'), mapping, subpath, mappingKey)
...
function resolveExportsTarget(baseUrl, target, subpath, mappingKey) {
    if (typeof target === 'string') {
        ...
        if (resolvedTarget === undefined) throw new ERR_INVALID_PACKAGE_TARGET(baseUrl.pathname, mappingKey, target)
        ...
        const reason =
            'request is not a valid subpath for the "exports" ' + `resolution of ${baseUrl.pathname}package.json`
        throw new ERR_INVALID_MODULE_SPECIFIER(mappingKey + subpath, reason)
    } else if (ArrayIsArray(target)) {
        if (target.length === 0) throw new ERR_PACKAGE_PATH_NOT_EXPORTED(baseUrl.pathname, mappingKey + subpath)
        let lastException
        for (const targetValue of target) {
            try {
                ...
                lastException = e
                if (e.code !== 'ERR_PACKAGE_PATH_NOT_EXPORTED' && e.code !== 'ERR_INVALID_PACKAGE_TARGET') throw e
            }
        }
        ...
        if (keys.some(isArrayIndex)) {
            throw new ERR_INVALID_PACKAGE_CONFIG(baseUrl, '"exports" cannot ' + 'contain numeric property keys.')
        }
        for (const p of keys) {
            switch (p) {
                case 'node':
        ...
        throw new ERR_PACKAGE_PATH_NOT_EXPORTED(baseUrl.pathname, mappingKey + subpath)
    } else if (target === null) {
        throw new ERR_PACKAGE_PATH_NOT_EXPORTED(baseUrl.pathname, mappingKey + subpath)
    }
    ...
    if (parent && parent.filename) {
        if (request[0] === '#') {
            ...
            if (pkg.data && pkg.data.imports !== null && pkg.data.imports !== undefined) {
                try {
                    const resolved = packageInternalResolve(request, pathToFileURL(parent.filename), cjsConditions)
                    return fileURLToPath(resolved)
                } catch (err) {
