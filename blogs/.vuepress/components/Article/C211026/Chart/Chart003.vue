...
export default {
  ...
  methods: {
    setup(ctx) {
      ...
      const nums = Array(300).fill().map((_, idx) => idx).sort(() => -1)
      ...
      // console.log(
      //   ctx.TAU / (ctx.TAU / this.memo.history.length)
      // )
    },
    async draw (ctx) {
      if (this.memo.history.length < (44850 * 0.19)) {
        ctx.noLoop()
        return null
      }
      let count = 1
