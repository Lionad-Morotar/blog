...
五月底，W 集中发版时碰到包括依赖下载失败导致的超时和 Runner Killed 问题（推测是内存超限被杀）。其中 Runner 被杀的问题尝试过修改 NodeJS 内存限制和增加预构建步骤，但是没能解决。过程及原因见第三节分析。为了从根本上解决之类资源不足问题，需要对插件编译工具链进行更新，迁移到速度更快、内存占用更低的 Rspack。

此次迁移较简单。后续平台可能提供脚手架工具来处理插件模板更新等杂项事务。

### 结果

以 [inventorytransfertask-web](https://git.ittx.com.cn/saas/prod/ttxrd/inventorytransfertask/inventorytransfertask-web) 为例，迁移到 Rspack 后，编译弹窗时，内存占用峰值从 4.7GB 降低到 1.3GB，降低到原先的 27%；编译速度从 132s 降低到 16s，速度提升 8 倍（12%）。
...
操作步骤分三步，1. 迁移插件弹窗编译工具链；2. 迁移插件编译工具；3. 配置插件外部依赖；

- 步骤 1：为了缓解 CI 服务器压力，解决 Runner 被杀问题，所有插件应至少完成步骤 1，同时能获得更快的编译速度。收益视弹窗数量而定，在 inventorytransfertask-web 插件中，4 个弹窗迁移后，能获得 3 分钟的收益，空弹窗的编译产物从 4MB 降低至 20KB。
- 步骤 2：进一步提高编译速度。收益视项目大小和复杂程度而定，预计在较小型的项目能获得 30s - 60s 收益。
- 步骤 3：将编译速度提高到常数级。在 eap-web 项目中，编译时间从 1分钟降低到 5s。

其中，步骤 2 的操作难度高于步骤 1，如果项目代码和依赖比较复杂则不建议迁移；步骤 3 会将插件内的 Vue、ElementPlus 依赖替换成平台提供的最新版本，所以风险主要来源于 Vue、ElementPlus 的 API 变化，一般情况下无风险。

完成迁移需要花费约 **30 分钟**，如果需要完成步骤 2 和步骤 3，需要预留 1 ~ 2 个小时的时间用于解决可能出现的适配问题。对于没有把握的项目，**可选择将业务代码复制到一个空插件模板，调试好后再覆盖回原先代码库这种方式。**

操作步骤完成后，配合周四（2023/06/07）发布的新插件 CI 模板，可以将一般插件项目（如 [eap-web](https://git.ittx.com.cn/saas/prod/cybercloud/eap/eap-web/-/pipelines)）CI 速度缩短到一分钟以内。

### 迁移前注意事项

查看项目内 package.json 中 CYBER_PLUGIN_TEMPLATE_VERSION 字段的值。它代表当前项目使用的插件模板的版本。

当**版本为 1.2.8** 时，可以按照以下两个步骤分别将 vue-cli（插件本体开发和编译） 和 webpack （插件弹窗编译）迁移到 rspack。如果插件版本更低（如**小于 1.2.8**），可能需要对相关代码做稍许调整。
...
1. 安装 Rspack 相关依赖：`pnpm install -D @rspack/cli @rspack/plugin-html`
2. 升级 vue-loader 到最新版：`pnpm update vue-loader@^17.2.2`
...
6. 替换 Rules，如果没有特殊需求可用一下 Rules 覆盖原先的 Rules
...
6. 编译配置文件增加 vue-loader 及常见 Rules
...
以下面的 provide 代码举例。它定义了全局变量 cyber。
...
在模板默认的 App.vue 文件中，能看到在 setup 中直接返回了 cyber 给模板使用。类似的代码会在编译时报错。
...
4. 替换编译配置中的 externals 设置，`.cybercloud/build/build-dialog.js`、`.cybercloud/build/build-dialog-lifecycle.js`、`.cybercloud/build/index.js` 三个文件分别对应插件弹窗、插件弹窗生命周期钩子函数和插件项目的编译配置，三个文件可以都改。修改的内容如下示例，可根据实际需要做调整。
...
5. 修改 index.html 模板代码。一是为了防止 Vue、ElementPlus 等依赖出现不兼容问题，二是为了不再使插件受 cybercloud-ui 发版影响。目前只能按照以下脚本 URL 锁定依赖的版本。
...
更多配置项可参考 [Rspack 官网](https://www.rspack.dev/zh/guide/introduction.html)或 [Rspack Gitlab](https://github.com/web-infra-dev/rspack)。
