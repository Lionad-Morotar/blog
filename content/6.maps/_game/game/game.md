...
* 2018：中央宣传部接管大部分国家新闻出版署的职能，负责包含游戏在内的出版物审批监管，原新闻出版广电总局不再保留。版号审批工作因机构整合陷入停滞，直到当年 12 月才恢复。8 月，《综合防控儿童青少年近视实施方案》
首次提出"实施网络游戏总量调控"，预示着监管部门要大幅压缩每年发放版号的数量。12 月，中央层面成立网络游戏道德委员会，对存量游戏展开评议整改。版号审批正式重启。
...
开发者通过构建同一个 Sonic 主题无限跑酷游戏，对比了三个主流 2D Web 游戏框架：

**Phaser** —— 商业项目的首选
- 性能与稳定性最佳，在 Safari（WebKit）上表现尤为出色，意味着 iOS 移动端兼容性最好
- AI 辅助优势显著，ChatGPT 对 Phaser 问题的回答准确率远高于其他两个框架
- 社区最大，资源丰富，寻求帮助更容易
- 支持 headless 模式在服务器端运行，便于开发多人游戏的状态同步
- 虽然 API 较冗长，但性能与稳定性的回报值得学习成本

**Kaboom.js (Klay)** —— 快速原型与教学利器
- 学习曲线最平缓，组件化设计让游戏设计迭代极快
- 适合游戏原型开发、Game Jam 和教学（甚至被用于儿童编程教育）
- 性能是三者中最差的，近期虽有优化但仍与 Phaser 差距明显
- 可扩展性有限，复杂项目会遇到瓶颈

**Excalibur** —— 有亮点但差异化不足
- 与 Phaser 同为面向对象范式，API 平均更简洁一些
- Debug 模式体验独特（浏览器扩展形式，在 DevTools 中切换和配置）
- 但场景状态管理需要手动处理（如返回游戏场景时敌人位置未重置），Phaser 则自动处理
- 总体感觉与 Phaser 相似度太高，性能、功能和生态均不如 Phaser，切换动力不足

**选型建议**：商业项目选 Phaser；快速原型或教学选 Kaboom.js；Excalibur 的差异化优势不足以支撑迁移成本。
