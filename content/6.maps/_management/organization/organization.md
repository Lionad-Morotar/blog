---
title: 组织
description: 团队组织架构、管理方法、沟通方式等。
original_path: content/6.maps/_management/organization.md
---

## 组织方法

* [Shape Up](/maps/_management/shape-up/shape-up)

## 组织理论

* [彼得原理](/maps/_management/organization/peter-principle) - 晋升悖论与管理者胜任力陷阱

## 目标管理

* OKR

## 组织切面

* [独立数据工程团队](/maps/_management/slice/standalone-data-engineering-team)

## 团队规模

#### 什么是"两个披萨"团队原则？

亚马逊创始人贝佐斯提出的管理原则：团队规模应控制在**两个披萨能喂饱的人数**范围内，即约 5-8 人。AI 出现以后，这一原则可能演变为"单披萨团队"——只需 2-3 人（含产品经理和设计师）即可完成过去需要更大团队的工作。
这反映了 AI 对组织形态的深刻影响：小团队的生产力正在被重新定义。

> #周刊摘录 见：[科技周刊第385期](https://www.ruanyifeng.com/blog/2026/02/weekly-issue-385.html)

## Agent Teams 与组织架构

#### Agentic Swarm 的组织架构陷阱

企业将 AI Agent 简单替换组织底层的"机器人版组织架构图"模式存在根本性缺陷。这种做法本质上是将现有企业层级中的底层替换为一群 AI Agent,保留人类作为监督者——这看似熟悉且令人安心,但恰恰是最大的危险信号。
真正的问题在于:当 Agent 能够自主协调和执行时,中层管理的"信息路由"价值被抽空,组织需要从根本上重新思考结构,而非简单替换。

我的看法是,Agent Teams 本身就不是为了构建人类组织的数字化复刻,那是对生产技术理解不够深刻的人的自嗨,实际上 Agent Teams 主要是解决工程问题,和上下文管理相关。

见：[Agentic Swarms Are an Org-Chart Delusion - Joan Westenberg](https://www.joanwestenberg.com/agentic-swarms-are-an-org-chart-delusion/)

## Domain

* [AI-accelerated Shadow IT](/maps/_management/shadow-it/ai-accelerated-shadow-it)
* [产能驱动开发](/maps/_management/capacity-driven-development/capacity-driven-development)

