...
讨论如何通过使用**新一代高性能工具**（如 `tsgo`、`oxlint` 和 `oxfmt`）来显著提升前端应用程序在 CI/CD 流水线中的运行效率。这些工具基于 Go 或 Rust 编写，比传统 Node.js 工具（如 ESLint、Prettier 和 TypeScript Compiler）更快，尤其适合在 CI 环境中运行。
