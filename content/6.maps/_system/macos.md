...
#### macOS `pfctl` anchor 不修改系统配置实现端口阻塞

macOS 的 `pf`（Packet Filter）主规则在 `/etc/pf.conf`，直接编辑风险高且会被系统升级覆盖。
更安全的做法是利用 anchor 机制：

1. 把规则写到临时文件
2. 用 `pfctl -a com.apple/<your-anchor> -f <file>` 加载到子锚点
3. `/etc/pf.conf` 中已有的 `anchor “com.apple/*”` 通配引用会让子锚点自动生效
4. 需要管理员权限时通过 `osascript` 弹出系统授权框，而不是自己保存密码或跑 sudo

清空规则时用 `pfctl -a com.apple/<anchor> -F rules`；启用 pf 时 tolerate `pfctl -e` 在已启用状态下返回的失败。

见：[pfctl man page](https://man.freebsd.org/cgi/man.cgi?query=pfctl)

#### devicectl：Xcode 15+ 的 iOS 真机管理 CLI

devicectl 是 Apple 官方的命令行设备管理工具，Xcode 15 起随 Xcode 提供（经 `xcrun devicectl` 调用），角色
相当于 Android 生态的 adb：无需打开 Xcode 图形界面即可与连接的 iOS 真机交互。典型能力包括安装 app
（`device install app`）、启动进程并实时抓控制台输出（`process launch --console`）、查看进程列表、读写
app 数据容器（`copy to/from`）。需要警惕一个危险参数：`copy to --remove-existing-content true` 清空的是
整个 app 数据容器（Documents 和 Library，包括存档与设置），而不只是目标路径，且没有安全的远程删除替代。
另一个差异：devicectl 启动的进程会绕过 iOS 看门狗，而点图标启动受看门狗约束，排查“命令行能跑、图标启动
被杀”类问题时先确认启动方式。
