...
发现某个 Node 进程占用了大量内存，分析进程后显示 `node .../llmworker.js`，猜测是 LM Studio 的服务。

因为 LM Studio 基于 Electron，使用 NodeJ 作为 Python ML Lib 的桥接，所以任务管理器里会显示 NodeJS 进程占的内存条。
...
# 1) 先找出”到底哪个 PID 在吃内存”
#
# macOS：RSS（常驻内存）单位是 KB；按 RSS 降序排序
ps -axo pid,ppid,rss,command \
  | grep -E “(node|npm|pnpm)” \
  | grep -vE “(grep|egrep)” \
  | sort -nrk3 \
  | head -20
#
# macOS：再用 top 交叉验证（rsize 更贴近 Activity Monitor 的”真实内存”感受）
top -l 1 -o rsize -stats pid,command,rsize,vsize,cpu,mem | head -30
#
# Linux 参考（如果你在 Linux 上排查）
# ps -eo pid,ppid,rss,cmd --sort=-rss | grep -E “(node|npm|pnpm)” | head -20

# 2) 确认进程细节与父进程（PID 假设为 28750）
ps -p 28750 -o pid=,ppid=,rss=,vsz=,command=
ps -p “$(ps -p 28750 -o ppid= | tr -d ' ')” -o pid=,command=

# 3) 看它到底在用什么文件/目录（可快速判断是否 Electron App / Node 服务）
lsof -nP -p 28750 | head -50

# 4) macOS 进一步定位”内存是怎么分配的”（需要更深挖时）
# vmmap -summary 28750 | head -60
```

#### 解析 `lsof`/`ps` 字段流输出并多源对齐

`lsof -nP -i -sTCP:LISTEN -FpcPn` 的输出是机器格式化的字段流，每行以单字母标签开头
（`p`=PID、`c`=命令、`P`=协议、`n`=地址），需要用状态机逐行聚合。

更复杂的是真实数据来自多个来源：`lsof` 给端口、`ps` 给进程元数据、第二次 `lsof -p` 给当前工作目录。
三份数据按 PID 对齐时，要注意：

- 一个 PID 可能监听多个地址/端口
- `n` 行可能是 `*:8080`、`127.0.0.1:8080`、`[::1]:443` 或已连接的 `a:1->b:2`（后者应丢弃）
- `ps` 输出按字节偏移切分比按固定列宽更鲁棒，因为 `lstart` 等字段含空格

解析时应做容错：任何一份来源失败都不应让整个列表崩溃。

见：[lsof man page](https://linux.die.net/man/8/lsof)
