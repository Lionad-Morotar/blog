...
OpenAI GPT-5.x 等推理模型在 `reasoning.effort != "none"` 时会直接拒绝 `temperature` 参数，这不是 bug，而是参数语义的切换。普通模型里 temperature 控制的是最终输出 token 的采样随机性；但在推理模型里，真正的随机性来自内部 reasoning token 的生成过程，最终答案往往是内部思考收敛后的确定性产物。你能调的不是"让它更发散或更保守"，而是 `reasoning.effort` 与 `max_output_tokens` 之间的分配比例——前者决定模型愿意想多久，后者决定总预算天花板，二者共同塑造了输出质量与成本结构。
