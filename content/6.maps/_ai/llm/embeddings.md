...
#### 稀疏向量依赖模型能力，商业 embedding API 默认不输出

embedding 按输出形态分 dense（稠密浮点向量，如 1536 维）和 sparse（稀疏词位映射，
类学习版 BM25，如 SPLADE/BGE-M3 的 sparse 头）。
商业 API（OpenAI、Gemini 等）通常只返回 dense，要拿到 sparse 必须自部署支持稀疏输出的模型
并经自定义 HTTP 端点调用。做 hybrid search 时 dense 路径可复用商业 API，
sparse 路径往往需额外的自托管模型——这是模型能力差异，不是协议差异。
