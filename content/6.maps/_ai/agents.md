...
#### Codex 的架构哲学：模型与 Harness 的协同演化

OpenAI 将 Codex 定义为 **Model + Harness + Surfaces** 三层结构。
关键洞见在于 harness（指令与工具集合）与模型的关系——
模型是在 harness 的"陪伴"下训练的，工具使用、执行循环、迭代验证
不是后期附加的能力，而是模型学习运作的内在方式。

这种设计打破了"先训练模型，再包装成产品"的传统分层思维：
harness 的形状会反过来塑造模型的规划方式、工具调用策略和故障恢复能力。

见：[How I think about Codex](https://simonwillison.net/2026/Feb/22/how-i-think-about-codex/)
...
## Hello-Agents：从 0 到 1 的系统化 Agent 教程

DataWhale 开源的系统性智能体学习教程，从基础理论到实际应用。涵盖智能体定义与发展史、大语言模型基础、ReAct/Plan-and-Solve/Reflection 等经典范式、Coze/Dify/n8n 低代码平台、AutoGen/AgentScope/LangGraph 框架，以及从 SFT 到 GRPO 的 Agentic-RL 训练。

* [Hello-Agents 在线教程](https://datawhalechina.github.io/hello-agents)：DataWhale 系统性智能体学习教程，涵盖 ReAct/Plan-and-Solve 范式、低代码平台、框架及 Agentic-RL 训练
