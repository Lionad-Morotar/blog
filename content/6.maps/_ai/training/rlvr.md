...
## Pass@1、Pass@k 与 Avg@N 的含义与张力

在 LLM 推理与代码生成评估里，这三个指标描述的是“采样-验证”策略的不同侧面。Pass@1 是对一个问题只采样一次、
答对才算对的单样本准确率；Pass@k 是对同一个问题独立采样 k 次，只要任意一次通过验证器就算解决；
Avg@N 则是 N 次独立 Pass@1 的平均值，反映模型单次生成的平均表现。

GRPO 等标准 RLVR 目标通常以提升 Pass@1 为主，但这容易让 policy 收敛到少数高奖励模式，导致输出多样性下降、
Pass@k 变差。NSR 和 ResRL 试图同时推高 Pass@1 与 Pass@k：前者通过强化负样本梯度保持探索，
后者进一步用投影残差保护共享语义，避免“为了惩罚错误而遗忘正确的前缀”。
...
NSR（Negative Sample Reinforcement）把 RLVR 的 policy gradient 按 advantage 的符号拆开：只保留 advantage ≤ 0 的样本，
并放大它们的负梯度，让模型主动远离错误 rollout。它和 PSR（Positive Sample Reinforcement）相对，后者只鼓励高分回答。

NSR 的出发点是缓解 GRPO 过度奖励正样本带来的多样性损失。但正/负回答往往共享大量 token，比如解题格式、中间推理步骤、
公共前缀。无差别地放大负样本梯度，会同时压低这些共享的正确语义，造成 Lazy Likelihood Displacement，
即正确轨迹的似然不升反降。这是 ResRL 引入投影残差进行“语义解耦”的直接动机。

见：[The Surprising Effectiveness of Negative Reinforcement in LLM Reasoning](https://arxiv.org/abs/2506.01347)
