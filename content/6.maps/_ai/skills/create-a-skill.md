...
对于涉及大量知识领域的复杂 Skill，单一扁平目录可能导致模型选择困难。Perplexity 在 Computer 的美国所得税 Skill 中采用了三层嵌套：将 IRS 的 1,945 个 section 按逻辑细分为 20 个主题域，每个域下再分具体 topic。[^1]
...
- **Index 层门槛极高**：~100 tokens/Skill 看似不多，但这是每个 session、每个用户始终支付的固定成本。Computer 的系统提示词中只注入 name 和 description，模型据此决定是否调用 `load_skill()`
- **Load 层持续计费**：Skill 加载后，~5000 tokens 的正文会在 compaction boundary 之前始终留在上下文中。一个对话若加载 3-5 个 Skill，成本会相乘。 fluff 会 degrade 其他 Skill 的能力和整体 agentic 表现
...
PostHog Wizard 团队跨多框架/语言维护一组「模型飞机」（Model Airplane）——**不是真生产应用，而是真应用的 simulacrum（拟像）**。关键特征是：外形完整（登录页、Stripe 接入、用户事件等关键业务表面都存在），但行为简化（Auth-shaped 但任意密码都能登录，无真实业务逻辑）。
...
**注入机制**：上下文服务（context service）把所有 Model Airplanes 扁平化为单个 Markdown 文件，作为 reference 与文档（plain text prose）一起装入 Skill。这是对渐进式披露 Level 3 资源层的一种特殊填充——把「成功集成的形状」也作为可挂载的领域知识。
