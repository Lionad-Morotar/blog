...
- [工作流程和反馈循环的设计](
#%E5%B7%A5%E4%BD%9C%E6%B5%81%E7%A8%8B%E5%92%8C%E5%8F%8D%E9%A6%88%E5%BE%AA%E7%8E%AF%E7%9A%84%E8%AE%BE%E8%AE%A1)
...
- [⚠️ 重要限制：
Skills 不跨平台同步](
#%EF%B8%8F-%E9%87%8D%E8%A6%81%E9%99%90%E5%88%B6-skills-%E4%B8%8D%E8%B7%A8%E5%B9%B3%E5%8F%B0%E5%90%8C%E6%AD%A5)
...
代码编写原则与 Skill 编写原则存在系统性差异。Perplexity Agents 团队指出，Python 之禅中至少一半的格言在 Skill 语境下完全错误或具有误导性。[^1]
...
不是每个领域都需要 Skill。Perplexity 团队的标准是：先让 agent 在没有 Skill 的情况下跑几个 hero query，看它是否 already doing a good job。[^1]
...
description 不是 Skill 的内部文档，而是模型的路由指令——这是 Skill 中最难写的一行。好的 description 不说"这个 Skill 做什么"，而说"何时加载这个 Skill"。[^1]
...
向 LLM 传达工作流程与向同事传达完全不同。对于存在一年以上的软件工具，只需提及其名称，LLM 就拥有所需信息。不要像在 README 中那样列出每个命令。[^1]
...
Perplexity 团队将写 evals 作为 Step 0，在写 Skill 内容之前完成。[^1] 评估用例的来源：
...
Perplexity 的三层成本模型与这里的分层完全对应，但更强调成本意识：[^1]
...
Skill 维护遵循 append-mostly 模式，gotchas 部分随时间累积最大价值。[^1]
...
[^1]: [Designing, Refining, and Maintaining Agent Skills at Perplexity](https://research.perplexity.ai/articles/designing-refining-and-maintaining-agent-skills-at-perplexity): Perplexity 研究团队公开的生产级 Skill 工程指南，涵盖 Skill 目录结构、三层成本模型、description 路由触发器设计、eval 驱动开发和 gotcha 飞轮维护机制。