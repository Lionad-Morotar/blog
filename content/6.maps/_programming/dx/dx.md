---
title: Developer Experience（DX）
description: 开发者体验（Developer Experience，DX）是指开发者在使用产品、工具、框架等过程中的感受和体验。它是用户体验（User Experience，UX）的一个子集，专注于开发者的需求和体验。
original_path: content/6.maps/_programming/dx.md
---

## Domain

- [GenAI for legacy codebases](/maps/_programming/dx/genai-for-legacy-codebases)
- [GenAI for forward engineering](/maps/_programming/dx/genai-for-forward-engineering)

## Cross Domain

- [Team based Instructions](/maps/_ai/prompt/team-based-instructions)
- [agents.md](/maps/_ai/vibe/agents-md)

## Gists

#### <Link type='h5' to='https://mgear-file.oss-cn-shanghai.aliyuncs.com/Developer%20Exception%20Engineering.html' source='https://www.swyx.io/developer-exception/' >《Developer Exception Engineering》</Link>

SWYX 认为社区在过去讨论 Developer Experience 的角度稍稍狭隘了。

* **能效提升**：提高代码效率、自动化、代码生成器、开箱即用。..
* **富文档**：起步文档、代码示例、可交互示例、API 清单、最佳实践、可搜索。..
* **开发者工具**：脚手架、编辑器插件、代码片段、代码展示、版本控制。..

这些东西对开发者来说时"友好的"，但是也很"简单"。也许是时候主动跳出站在开发者的角度思考 DX 的舒适区，开始转向终端用户。也许可以尝试保证这些东西：

* **服务可用**：产品数据页面和报错页面会说谎吗？...
* **反馈有效**：是否及时有效的解决了客户反馈？...
* **功能完整**：是否发布了不完整甚至需要返工的功能？...
* **产品路线完整**：对产品路线的规划是否清晰合理？...
* **计价合理**：用户会对你的报价产生疑惑么，它是否是可计算和比较的？...
* **兼容性**：废弃的 API 是否合理？...
* **报错优雅**：出错后是否可调试？...
* **访问控制**：客户有审计日志或访问控制的权力吗？...

正如面对漏水的桶下雨解决不了问题，如果只是一直站在开发者的角度思考 DX，对解决品牌满意度下降、客户流失这些"异常"没有办法带来帮助。此外，根据康威定律，一个不合理的 DX 会反映出公司的组织架构的不合理之处，这值得引起注意。

![The Experience of Developer | https://www.swyx.io/developer-exception/](https://mgear-image.oss-cn-shanghai.aliyuncs.com/image/other/20210917170405.png)

#### pnpm 项目的模块解析仍可能被主目录污染

pnpm 的 isolated `node_modules` 只是把依赖以 symlink 形式隔离到 `.pnpm`，并不意味着运行时模块解析完全隔离。当工具链使用 `pathToFileURL(cwd)` 生成 base URL 时，
`file:///project/subdir` 这种没有 trailing slash 的 URL 会被 WHATWG URL 标准视为指向一个”文件”，
于是 `new URL(“./node_modules/...”, base)` 实际从父目录开始查找。如果父目录及以上存在同名但大版本不同的包，
解析器会优先命中最近的那个，导致项目加载到错误版本。mlly 的 `_resolve` 在 v1.8.2 及之前受此影响，
Nuxt 3 项目若位于 Nuxt 4 项目（或主目录存在 Nuxt 4）的子目录中，就会加载到 Nuxt 4 的 `initNuxt`，
进而抛出 `Cannot read properties of undefined (reading 'builder')` 这类不直观的错误。

见：[mlly issue #158](https://github.com/unjs/mlly/issues/158)

#### 前端测试应集成 `cargo check` 等构建验证

在 Tauri 这类”前端 + 原生后端”的混合项目中，前端测试全绿不代表产物能构建成功。Rust 侧的编译错误、
TOML/JSON 配置语法错误、capability 声明不匹配都可能被前端测试遗漏。

推荐把 `cargo check --manifest-path src-tauri/Cargo.toml` 直接加入 `pnpm test` 或 CI 脚本。
这样每次跑测试时同时验证前后端编译状态，避免”测试通过但构建失败”的滞后返工。

见：[Tauri Development](https://v2.tauri.app/develop/)

