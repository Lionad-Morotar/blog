...
用户态 VFS 库（memfs、mock-fs）存在根本限制：模块解析器无法被 hook（`import('./config.json')` 绕过用户态 mock）；依赖 `Module._resolveFilename` 等不稳定私有 API；提前捕获的 `fs.readFileSync` 引用会绕过 patch；原生模块加载（dlopen）需要真实文件路径。原生实现以 Provider/Mount/Overlay 三层架构解决了这些问题——仅拦截 VFS 中存在的路径，其余透传到真实文件系统。
