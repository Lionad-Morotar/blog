...
## Brief
TCR（Test && Commit || Revert）是一种从测试驱动开发（TDD）衍生出的编程工作流，其核心规则为：每次代码变更后，若测试通过则提交修改，若失败则回滚变更。该流程通过极小的、持续的代码迭代强化开发纪律，
例如遵循YAGNI（无需预先建造）和KISS（保持简单）原则。实现TCR仅需在代码库中通过脚本自动化这一测试-提交/回滚的循环。TCR最初由Kent Beck提出，当前在探索基于生成式AI（GenAI）构建软件时，
可作为优化开发流程的候选方案进行评估。
