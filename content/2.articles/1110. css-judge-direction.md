---
title: ❓ 用纯CSS判断鼠标进入的方向
description: 本文介绍了使用CSS判断鼠标进入区域方向的原理，并且在codepen笑脸demo的基础上实现了一个眼睛跟随鼠标位置进行转动的效果。
---
...
表单 Form 元素倒是可以通过当子组件处于 :focus 状态时，标记其自身为 :focus-within 状态。不过有个问题，它不能告诉我们到底是哪个子组件处于 :focus ，
