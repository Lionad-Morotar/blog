...
const mdFileDirs = require('./undone-files.js').files

// 移动字体文件到临时目录
gulp.task('move-font-file', () => {
  return gulp
    .src('blogs/.vuepress/public/fonts/NotoSerifSC-Light.otf')
    .pipe(gulp.dest('tmp'))
})
...
gulp.task('font', ['move-font-file'], () => {
  ...
  gulp
    ...
    // 将 MD 文件内容拼接到 HTML 中
    .pipe(
      $.replace(/[.\s\S]*/i, function(match, group, offset, filecontent) {
        return fontHTML(
          match.replace(/\s/g, '').replace(/[a-zA-Z0-9]|[`/\\]/g, '')
        )
      })
    )
      ...
      (function() {
        return through.obj(function(file, enc, cb) {
          ...
          cmd.get(cmdSubFont, () => {
            ...
            Promise.all([changeCSSContentAndName, createVueComponents]).then(
              () => {
